...
export default class GameScene extends Phaser.Scene {
  ...
  // ── Bonus Processors ──────────────────────────────────
  // Real logic lives in src/BonusEngine.js (pure, tested). These wrappers just
  // supply the scene context. All three mutate handResult in place.

  _applyStaffEffects(handResult, gameState) {
    return BonusEngine.applyStaff(handResult, gameState, {
      staff: this.baseball.getStaff(),
    });
  }

  _applyLineupEffects(handResult, gameState) {
    return BonusEngine.applyLineup(handResult, gameState, {
      effects: this.rosterManager.getActiveLineupEffects(),
      batter: this.rosterManager.getCurrentBatter(),
      discardCount: this.discardCount,
    });
  }

  _applySynergyEffects(handResult, gameState) {
    return BonusEngine.applySynergies(handResult, gameState, {
